...
#   Маска 1
seeds = [(320, 500), (460, 1120), (1330, 1150)]#,(740, 540)]#,(630, 40)]
...
segmented_region1 = segmentation_utils.region_growingHSV(image_hsv, seeds, threshold)

#   Маска 2
seeds1 = [(790, 520),(650, 95)]
# координаты для графика
x1 = list(map(lambda x: x[1], seeds1))
y1 = list(map(lambda x: x[0], seeds1))
# порог похожести цвета региона
threshold1 = 82
# находим сегментацию используя метод из segmentation_utils
segmented_region2 = segmentation_utils.region_growingHSV(image_hsv, seeds1, threshold1)

#   Маска результирующая
segmented_region = segmented_region2 + segmented_region1
...
plt.scatter(x1, y1, marker="x", color="red", s=200)
...
lut = lambda i: 255 - i
invert_image = lut(binary_image)
plt.imshow(invert_image, cmap='gray')
binary_image = invert_image
